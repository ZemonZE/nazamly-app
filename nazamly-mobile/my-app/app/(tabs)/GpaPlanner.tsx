...
export default function GpaPlannerScreen() {
  ...
  const generateTargetPlan = async (
    targetCGPA: number,
    currentCourses: any[],
    token: string,
  ) => {
  ...
  useFocusEffect(
    useCallback(() => {
      AsyncStorage.getItem(STORAGE_KEY)
        ...
        .catch((err) =>
          console.error("[GpaPlanner] Failed to load profile:", err),
        );

      AsyncStorage.getItem(COURSES_STORAGE_KEY)
        .then((saved) => {
          if (!saved) return;
          try {
        ...
        .catch((err) =>
          console.error("[GpaPlanner] Failed to load courses:", err),
        );
        ...
        (async () => {
          try {
            ...
            if (backendCourses && backendCourses.length > 0) {
              ...
              const localSaved =
                await AsyncStorage.getItem(COURSES_STORAGE_KEY);
              if (!localSaved) {
                setCourses(mapped);
  ...
  const saveProfile = useCallback(async () => {
    ...
    AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(data)).catch((err) =>
      console.error("[GpaPlanner] save profile error:", err),
    );
    setProfile(data);
  ...
  const editProfile = () => {
    ...
    setCourses([]);
    setGrades({});
    AsyncStorage.removeItem(COURSES_STORAGE_KEY).catch((err) =>
      console.error("[GpaPlanner] remove courses error:", err),
    );
  };
  ...
  const handleGradeChange = (id: string, value: string) => {
    ...
    ).catch((err) => console.error("[GpaPlanner] save grades error:", err));
  };
  ...
  const handleFlowDone = (
    ...
    ).catch((err) => console.error("[GpaPlanner] save flow error:", err));
  };
  ...
  const addManualCourse = async () => {
    ...
    ).catch((err) => console.error("[GpaPlanner] save add error:", err));
  ...
  const updateManualCourse = (
    ...
    ).catch((err) => console.error("[GpaPlanner] save update error:", err));
  };
  ...
  const removeManualCourse = async (id: string) => {
    ...
    ).catch((err) => console.error("[GpaPlanner] save remove error:", err));
  ...
  // ── Onboarding ──
  const s = createGpaStyles(isTablet);
            ...
            {courses.length > 0 ? (
              ...
              >
                ...
                <Text style={[s.emptyCoursesText, { color: colors.textMuted }]}>
                  Select &quot;From History&quot; above to load courses from a
                  previous transcript
                </Text>
              </View>
            )}
...
const createGpaStyles = (isTablet = false) => {
