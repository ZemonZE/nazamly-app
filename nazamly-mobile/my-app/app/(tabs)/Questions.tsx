...
const getSubFolderFiles = async (
  ...
  const res = await fetch(
    `${API_URL}/api/course-materials/${safeCode}/files/${subFolderType}`,
    { headers: { Authorization: `Bearer ${token}` } },
  );
  const json = await res.json();
...
const generateExamStream = async (
  opts: any,
  token: string,
...
export default function QuestionsScreen() {
  ...
  const [selectedCourse, setSelectedCourse] = useState<CourseMaterial | null>(
    null,
  );
  ...
  // ── Step 3: Exam Type ──
  const [examType, setExamType] = useState<"Quiz" | "Midterm" | "Final">(
    "Quiz",
  );
  ...
  const [archiveRevealed, setArchiveRevealed] = useState<
    ...
    (async () => {
      ...
      try {
        ...
        const materialMap = new Map(
          (materials || []).map((c: any) => [c.courseCode, c]),
        );
  ...
  const handleFetchArchive = async () => {
    ...
    try {
      const token = await user.getIdToken();
      const response = await getArchiveQuestions(
  ...
  const getOptionStyle = (q: AIQuestion, option: string, qIdx: number) => {
    ...
    if (!aiSubmitted) {
      return selected === option
        ? { backgroundColor: colors.indigo + "20", borderColor: colors.indigo }
        : {};
    }
    if (option === q.correctAnswer)
      return { backgroundColor: "#22c55e20", borderColor: "#22c55e" };
    if (selected === option && selected !== q.correctAnswer)
      return { backgroundColor: "#ef444420", borderColor: "#ef4444" };
    return {};
  };
    ...
    <SafeAreaView style={s.container}>
      ...
      <View style={s.tabBar}>
        <TouchableOpacity
          style={[
            s.tabBtn,
            activeTab === "generate" && { backgroundColor: colors.indigo },
          ]}
        ...
        <TouchableOpacity
          style={[
            s.tabBtn,
            activeTab === "history" && { backgroundColor: colors.indigo },
          ]}
        ...
        >
          ...
          />
          <Text
            style={[s.tabBtnText, activeTab === "history" && { color: "#fff" }]}
          >
            My Quizzes
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[
            s.tabBtn,
            activeTab === "archive" && { backgroundColor: colors.indigo },
          ]}
        ...
        >
          <MaterialCommunityIcons
            name="archive"
            size={16}
            color={activeTab === "archive" ? "#fff" : colors.textMuted}
          />
          <Text
            style={[s.tabBtnText, activeTab === "archive" && { color: "#fff" }]}
          >
            Archive
          </Text>
        </TouchableOpacity>
          ...
          <>
            ...
            {!aiLoading && aiQuestions.length === 0 && !aiError && (
              <View style={[s.card, { backgroundColor: colors.card }]}>
                ...
                {coursesLoading ? (
                  ...
                  <Text style={{ color: "#ef4444", padding: 10 }}>
                    {coursesError}
                  </Text>
                ) : courses.length === 0 ? (
            ...
            {aiLoading && (
              <View style={s.loadingSection}>
                ...
                <Text style={[s.loadingMsg, { color: colors.textMuted }]}>
                  {aiStatusMessage}
                </Text>
              </View>
            )}
            ...
            {aiError && !aiLoading && aiQuestions.length === 0 && (
              ...
              >
                ...
                <Text style={[s.errorTitle, { color: colors.textPrimary }]}>
                  Generation Failed
                </Text>
                ...
                <TouchableOpacity
                  style={[s.retryBtn, { backgroundColor: colors.indigo }]}
                  onPress={handleReset}
                >
            ...
            {aiQuestions.length > 0 && (
              <>
                ...
                {aiSubmitted && (
                  <View
                    style={[s.scoreCard, { backgroundColor: colors.indigo }]}
                  >
                    ...
                    <Text style={s.scoreDetail}>
                      Correct: {aiScore} / {aiQuestions.length}
                    </Text>
                    <TouchableOpacity
                      style={s.newExamBtn}
                      onPress={handleReset}
                    >
                      ...
                      />
                      <Text
                        style={[s.newExamBtnText, { color: colors.indigo }]}
                      >
                        Generate Another
                      </Text>
                    </TouchableOpacity>
                ...
                {!aiSubmitted && (
                  <View
                    style={[s.progressBar, { backgroundColor: colors.card }]}
                  >
                    <Text
                      style={[s.progressText, { color: colors.textPrimary }]}
                    >
                      ...
                      <View
                        style={[
                          s.progressFill,
                          {
                            backgroundColor: colors.indigo,
                            width:
                              `${totalQuestions > 0 ? Math.round((answeredCount / totalQuestions) * 100) : 0}%` as any,
                          },
                        ]}
                      />
                    </View>
                ...
                {aiQuestions.map((q, idx) => {
                  ...
                  return (
                    <View
                      key={q._id || idx}
                      style={[s.questionCard, { backgroundColor: colors.card }]}
                    >
                      {/* Header badges */}
                      <View style={s.qBadgeRow}>
                        {q.difficulty != null && (
                          <View
                            style={[
                              ...
                              {
                                backgroundColor:
                                  getDifficultyColor(q.difficulty) + "18",
                              },
                            ]}
                          >
                            <Text
                        ...
                        <View
                          style={[
                            s.typeBadge,
                            {
                              backgroundColor: isTF ? "#14b8a618" : "#6366f118",
                            },
                          ]}
                        >
                          <Text
                      ...
                      <Text style={[s.qNumber, { color: colors.textMuted }]}>
                        Q{idx + 1}
                        {q.derivedFromConcept
                          ? ` · ${q.derivedFromConcept}`
                          : ""}
                      </Text>
                      <Text style={[s.qText, { color: colors.textPrimary }]}>
                        {q.questionText}
                      </Text>
                ...
                {!aiSubmitted && (
                  ...
                  >
                    <Feather
                      name={allAnswered ? "check-circle" : "alert-circle"}
                      size={18}
          ...
          <View style={[s.card, { backgroundColor: colors.card }]}>
            <View style={s.cardHeaderRow}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={[s.cardDesc, { color: colors.textMuted }]}>
                  Review your past AI-generated quizzes.
                </Text>
              </View>
            </View>
              ...
              quizHistory.map((attempt) => (
                ...
                >
                  <View style={s.historyTop}>
                    <Text style={[s.historyDate, { color: colors.textMuted }]}>
                  ...
                  <Text
                    style={[s.historyCourseName, { color: colors.textPrimary }]}
                  >
                    {attempt.courseId?.courseName || "Unknown Course"}
                  </Text>
                  <Text
                    style={[s.historyCourseCode, { color: colors.textMuted }]}
                  >
        ...
        {selectedHistoryQuiz && activeTab === "history" && (
          <>
            <TouchableOpacity
              ...
              onPress={() => setSelectedHistoryQuiz(null)}
            >
              <Feather name="arrow-left" size={18} color={colors.textPrimary} />
              <Text style={[s.backBtnText, { color: colors.textPrimary }]}>
                Back to Quizzes
              </Text>
            </TouchableOpacity>
            ...
            <View style={[s.scoreCard, { backgroundColor: colors.indigo }]}>
              ...
              <Text style={s.scoreTitle}>
                {selectedHistoryQuiz.courseId?.courseName || "Quiz Review"}
              </Text>
              ...
              <Text style={s.scoreDetail}>
                {selectedHistoryQuiz.score}/{selectedHistoryQuiz.totalQuestions}{" "}
                Correct ·{" "}
                {new Date(selectedHistoryQuiz.createdAt).toLocaleDateString()}
              </Text>
            </View>
            ...
            {selectedHistoryQuiz.questionsSnapshot.map((q, idx) => (
              ...
              >
                ...
                {q.explanation && (
                  <View
                    style={[s.explanationBox, { backgroundColor: colors.bg }]}
                  >
                    <Text
                      style={[
                        s.explanationText,
                    ...
                    {q.correctAnswer && (
                      <Text
                        style={[s.correctAnswerText, { color: colors.indigo }]}
                      >
                        Correct: {q.correctAnswer}
                      </Text>
                    )}
          ...
          <View style={[s.card, { backgroundColor: colors.card }]}>
            <View style={s.cardHeaderRow}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={[s.cardDesc, { color: colors.textMuted }]}>
                  Review midterm and final questions.
                </Text>
              </View>
            </View>
                      ...
                      return (
                        ...
                        >
                          <View
                            style={[
                              s.lectureCheck,
                              isSelected && { backgroundColor: colors.indigo },
                            ]}
                          >
                            {isSelected && (
            ...
            {selectedLectures.length > 0 && (
              <TouchableOpacity
                style={[
                  s.generateBtn,
                  { backgroundColor: colors.indigo, marginTop: 16 },
                ]}
              ...
              <View style={{ marginTop: 16 }}>
                {archiveMidterms.length === 0 && archiveFinals.length === 0 ? (
                  <View style={s.emptyState}>
                    ...
                    <Text style={[s.emptyDesc, { color: colors.textMuted }]}>
                      No past exam questions for this selection.
                    </Text>
                  </View>
                ) : (
                  <>
                      ...
                      <View style={{ marginBottom: 18 }}>
                        ...
                        {archiveMidterms.map((q, idx) => {
                          ...
                          return (
                            ...
                            >
                              <Text
                                style={[s.qNumber, { color: colors.textMuted }]}
                              >
                                Midterm Q{idx + 1} {q.year ? `· ${q.year}` : ""}
                              </Text>
                              <Text
                                style={[s.qText, { color: colors.textPrimary }]}
                              >
                                {q.questionText}
                              </Text>
                              {q.options && q.options.length > 0 && (
                                <View style={s.optionsGrid}>
                                  {q.options.map((option, optIdx) => (
                                    <TouchableOpacity
                                      ...
                                      onPress={() =>
                                        handleArchiveSelectAnswer(key, option)
                                      }
                                      disabled={resolved}
                                    >
                                      <View
                              ...
                              {!resolved && (
                                <TouchableOpacity
                                  ...
                                  onPress={() => handleArchiveReveal(key)}
                                >
                                  <Text
                                    style={[
                                      s.revealBtnText,
